...
def main(train_path,test_path,step_save_name,step):
    ...
    blurrer = tfs.GaussianBlur((3, 3))

    rnn_model=torch.load("original_best.tar")
    ...
    for epoch in range(1):
        ...
        for data in test_loader:  # Iterate over your training dataset
            ...
            obs = inputs.float()
            bright_im = tfs.ColorJitter(brightness=1)(obs.unsqueeze(1))  # 随机从 0 ~ 2 之间亮度变化，1 表示原图
            # save_image(bright_im,'bright_im.png')
            if torch.rand(1).item() < 0.5:
                bright_im = 1 - bright_im

                # save_image(gblur,str(iblur)+'gblur.png')
            zoutputs = rnn(bright_im)
